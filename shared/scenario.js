...
import { A11Y_COPY, guardrailHtml, escalateHtml, consentHtml, actionOutcomeHtml } from './a11y.js';
...
export const CITATIONS = {
  ...
  c2: { chip: '📎 method', src: 'Methodology', docId: 'retirement-guideline',
    body: `Industry retirement guideline (10x salary by ${F.retirementAge}), 4 scenario simulation · assumptions shown on request.` },
  ...
  c5: { chip: '📎 source', src: 'Source', docId: 'volatility-education',
    body: `Market education library: "Time in the market vs timing the market" · educational content, not advice.` },
...
/**
 * A citation is a disclosure, not a new context: a real button carrying
 * aria-expanded/aria-controls, with the revealed panel emitted immediately
 * after it in the DOM. Trigger and panel are produced together precisely so
 * they cannot drift apart as the copy is edited.
 */
const cite = id => {
  const c = CITATIONS[id];
  return `<button type="button" class="citation" data-cite="${id}" aria-expanded="false" aria-controls="${id}" aria-label="${A11Y_COPY.citeShow(c.src)}">${c.chip}</button>` +
    `<div id="${id}" class="cite-panel" role="group" aria-label="${c.src}"><div class="src">${c.src}</div>${c.body}</div>`;
};
...
export function buildScenario(p) {
  return {
    ...
    s1_stockAsk: {
      ...
      events: [
        { type: 'coach',
          html: `${ADVICE_LINE.refusal.copy}<br><br>${ADVICE_LINE.refusal.redirect}${guardrailHtml(ADVICE_LINE.refusal.banner)}`,
    ...
    s1_retirement: {
      ...
      events: [
        { type: 'coach', claims: 4,
          html: `Let's look together. Based on your linked accounts, you're ${F.age}, your employer matches up to ${F.matchCapPct}%, and you're contributing <b>${F.contributionPct}%</b> to your 401(k) with a balance of <b>${$(F.balance401k)}</b>. ${cite('c1')}
        ...
        { type: 'coach', noFeedback: true,
          html: `One more thing — you said you feel behind. That context helps me coach you better, but it's <i>my</i> inference, not something you told me to keep.
            ${consentHtml({ ask: CONSENT_COPY.ask('retirement adequacy is your main concern'), yes: CONSENT_COPY.yesButton, no: CONSENT_COPY.noButton })}`,
    ...
    s1_impact: {
      ...
      events: [
        { type: 'coach', claims: 2,
          html: `Raising your contribution from ${F.contributionPct}% → ${F.proposedContributionPct}% captures the full match, projected to close your gap from ${F.replacementNowPct}% → <b>~${F.replacementAfterPct}%</b>. That's roughly <b>+${$(F.annualIncrease)}/year</b> in combined contributions. ${cite('c3')}
            <div class="action-card" id="ac1" role="group" aria-labelledby="ac1-head">
              <div class="ac-head" id="ac1-head">${AUTONOMY.actionCardHead}</div>
              <div class="ac-body">
              ...
              <div class="ac-btns">
                ...
                <button type="button" data-x="review">Review details first</button>
    ...
    actionCard: {
      approvedHtml: actionOutcomeHtml(`✓ Approved — submitted to your plan. Confirmation #${F.confirmationNumber}. Reversible until Thursday.`),
      approvedAnnouncement: `Change approved. Confirmation number ${F.confirmationNumber}. Reversible until Thursday.`,
      ...
      events: [
        { type: 'coach', claims: 1,
          html: `I hear you — weeks like this are genuinely stressful, and you're not alone in feeling it.<br><br>
            I can't tell you whether to sell — that's a decision that deserves a licensed professional, especially when it's driven by how you're feeling right now. What I can show you: your plan was built expecting weeks like this one, and historically, missing just the ${F.missedBestDays} best market days over ${F.missedBestDaysYears} years cut returns roughly in half. ${cite('c5')}
            ${escalateHtml(ESCALATION_OFFER.copy, ESCALATION_OFFER.button)}`,
    ...
    s2_fund: {
      ...
      events: [
        { type: 'coach', claims: 4,
          html: `Great instinct — this is exactly the right next step. You have <b>${$(F.savings)}</b> in savings, and your essential expenses run about <b>${$(F.expensesMonthly)}/month</b> — roughly ${F.coverageMonths} months of coverage, where the guideline is 3–6 months. ${cite('c6')}
